...
public class PatronStatusViewModel extends BaseViewModel implements NetworkInterface {
    ...
    private SingleLiveEvent<PatronInfo> mPatronInfo = new SingleLiveEvent<>();
    ...
    public void getPatronInfo(String typedText) {
        if (!TextUtils.isEmpty(typedText)) {
            setIsLoding(true);
            mAppRemoteRepository.getPatronStatus(this, AppSharedPreferences.getInstance(getApplication())
    ...
    public void onCallCompleted(Object model) {
        ...
        try {
            if (model instanceof PatronInfo) {
                PatronInfo patronInfo = (PatronInfo) model;
                setPatronInfo(patronInfo);
...
//            }
    ...
    public SingleLiveEvent<PatronInfo> getPatronInfo() {
        return mPatronInfo;
    }
    
    public void setPatronInfo(PatronInfo patronInfo) {
        this.mPatronInfo.setValue(patronInfo);
    }
